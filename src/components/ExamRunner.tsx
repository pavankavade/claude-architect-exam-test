...
function ExamRunnerContent() {
  ...
  useEffect(() => {
    try {
      const saved = localStorage.getItem("cca_nextjs_exam_state_v5");
      if (saved) {
  ...
  useEffect(() => {
    ...
    try {
      ...
      localStorage.setItem("cca_nextjs_exam_state_v5", JSON.stringify(data));
    } catch (e) {
  ...
  const handleResetExam = () => {
    if (confirm("Are you sure you want to reset all answers and progress?")) {
      ...
      setShowSummary(false);
      setCurrentIndex(0);
      localStorage.removeItem("cca_nextjs_exam_state_v5");
    }
  };

  return (
    <div className="h-[calc(100vh-64px)] flex flex-col justify-between bg-[#090e17] overflow-hidden">
      {/* Top Progress Bar */}
      ...
      {/* Main Workspace (125% Proportional Feel by Default) */}
      <div className="max-w-[1520px] mx-auto w-full px-4 sm:px-6 py-3.5 flex gap-5 flex-1 items-stretch overflow-hidden">
        {/* =========================================
            LEFT DOCKED SIDEBAR (SPACIOUS & DOCKED)
        ========================================= */}
        {sidebarOpen && (
        ...
        {/* =========================================
            CENTERED QUESTION WORKSPACE (125% SCALED PROPORTIONS)
        ========================================= */}
          ...
          {!showSummary ? (
            currentQ && (
              <div className="space-y-3">
                ...
                <div className="flex items-center justify-between flex-wrap gap-2">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <button
                      ...
                      className="inline-flex items-center gap-1.5 px-3 py-1 rounded-lg bg-[#131d2e] border border-[#23344e] hover:border-sky-500 text-xs font-semibold text-sky-400 hover:text-white transition-colors"
                    >
                ...
                {/* Audio Bar */}
                <div
                  className={`px-3.5 py-2 rounded-2xl border flex items-center justify-between gap-3 transition-all ${
                    isSpeaking
                ...
                >
                  ...
                  <div className="flex items-center gap-2 flex-shrink-0">
                    <button
                      ...
                      className="px-3 py-1.5 rounded-lg text-xs font-bold bg-sky-500 hover:bg-sky-400 text-slate-900 shadow-sm flex items-center gap-1.5 transition-all hover:scale-105"
                    >
                    ...
                    <button
                      ...
                      className="px-3 py-1.5 rounded-lg text-xs font-semibold bg-[#1c2a42] hover:bg-[#23344e] border border-[#23344e] text-slate-200"
                    >
                      <Play className="w-3.5 h-3.5 text-indigo-400 inline mr-1" /> + Choices
                    </button>
                  ...
                  {currentQ.options.map((opt) => {
                    ...
                    return (
                      <button
                        ...
                        className={`w-full text-left py-2.5 px-3.5 rounded-xl border-2 transition-all cursor-pointer flex items-center gap-3.5 ${cardBorder}`}
                      >
                        <div
                        ...
                        <div className="flex-1 text-xs sm:text-[14px] text-slate-200 leading-snug flex items-center justify-between flex-wrap gap-2">
                          ...
                          {isAnswered && opt.letter === currentQ.correct && (
                            <span className="inline-flex items-center gap-1 text-xs font-bold text-emerald-400 bg-emerald-950/60 border border-emerald-600/60 px-2.5 py-0.5 rounded-full">
                              <Check className="w-3 h-3" /> Correct Answer
                            </span>
                          )}

                          {isAnswered && opt.letter === chosen && opt.letter !== currentQ.correct && (
                            <span className="inline-flex items-center gap-1 text-xs font-bold text-rose-400 bg-rose-950/60 border border-rose-600/60 px-2.5 py-0.5 rounded-full">
            ...
            <div className="space-y-4 animate-fadeIn py-4 max-w-2xl mx-auto text-center">
              <div className="p-8 rounded-3xl bg-[#131d2e] border border-[#23344e] shadow-2xl">
                <div className="text-5xl sm:text-6xl font-black bg-gradient-to-r from-sky-400 via-indigo-300 to-emerald-400 bg-clip-text text-transparent mb-1">
                ...
                {/* Metric Cards */}
                <div className="grid grid-cols-4 gap-2.5 max-w-lg mx-auto mb-5 text-center">
      ...
      ========================================= */}
      <footer className="h-14 bg-[#090e17]/95 backdrop-blur-md border-t border-[#23344e] px-4 sm:px-6 flex items-center justify-between shadow-xl z-30">
        <div className="flex items-center gap-2.5">
          <button
            onClick={() => handleNavigate(-1)}
            disabled={currentIndex === 0}
            className="px-3.5 py-1.5 rounded-lg border border-[#23344e] bg-[#131d2e] hover:bg-[#1c2a42] disabled:opacity-30 disabled:cursor-not-allowed text-xs font-bold text-slate-200 flex items-center gap-1.5"
          >
          ...
          <button
            ...
            className="px-4 py-1.5 rounded-lg border border-[#23344e] bg-sky-500 hover:bg-sky-400 disabled:opacity-30 disabled:cursor-not-allowed text-xs font-bold text-slate-900 flex items-center gap-1.5 shadow-sm"
          >
        ...
        <div className="flex items-center gap-3.5">
          <label className="hidden md:flex items-center gap-2 text-xs font-medium text-slate-300 cursor-pointer">
            <input
              type="checkbox"
              checked={autoRead}
          ...
          {isAnswered && (
            <button
              ...
              className="px-3 py-1 rounded-lg border border-[#23344e] bg-[#131d2e] hover:bg-[#1c2a42] text-xs font-semibold text-slate-400 hover:text-white"
            >
              Clear Selection
            </button>
          )}
